...
def preprocess(split, val_split, test_split):
    ...
    y_split = split.loc[:, 'g0_arrest']
    split = split.loc[:, split.columns != 'g0_arrest']

    y_val_split = val_split.loc[:, 'g0_arrest']
    val_split = val_split.loc[:, val_split.columns != 'g0_arrest']

    y_test_split = test_split.loc[:, 'g0_arrest']
    ...
    return (X_train_split, y_split), (X_val_split, y_val_split), (X_test_split, y_test_split) # , y_split
...
def preprocess_clinical(args):
    ...
    for kfold in range(args.k_fold):
        ...
        X_test = pd.concat([X_test.reset_index(drop=True), y_test.reset_index(drop=True)], axis=1)
        # pdb.set_trace()
        ...
        for split_name, df in dataframes:
            ...
            if split_name == 'train' or split_name == 'validation':
                ...
                df.to_csv(os.path.join(cv_dir, f"{split_name}.csv"))

            elif split_name == 'test':

                test_dir = f'{output_dir_root}test/'
                os.makedirs(test_dir, exist_ok=True)
                df.to_csv(os.path.join(test_dir, f"{split_name}.csv"))
