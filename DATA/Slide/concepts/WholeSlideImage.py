...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def drawPatchBoxes(self, scores, coords, vis_level=-1, 
                   top_left=None, bot_right=None,
                   patch_size=(224, 224), 
                   blank_canvas=False, canvas_color=(220, 20, 50), alpha=0.4, 
                   blur=False, overlap=0.0, 
        ...
        for idx in range(len(coords)):
            ...
            if score >= threshold:
                ...
                # color block (cmap applied to attention block)
                # color_block = (cmap(raw_block) * 255)[:,:,:3].astype(np.uint8)
                shape = (raw_block.shape[0], raw_block.shape[1], 3)
                color_block = np.full(shape, (0, 128, 128) )
                # (0.5, 0, 0.5)
                # pdb.set_trace()
                # Create a mask for the center area to be transparent
                center_mask = np.zeros((patch_size[1], patch_size[0]), dtype=bool)
                center_radius = patch_size[0] // 5  # Adjust this value to change the size of the transparent center
                center_mask[center_radius:-center_radius, center_radius:-center_radius] = True

                # Apply the color block only around the edges
                img_block[~center_mask] = color_block[~center_mask]
                if segment:
                    # tissue mask block
                    # pdb.set_trace()
                    mask_block = tissue_mask[coord[1]:coord[1]+patch_size[1], coord[0]:coord[0]+patch_size[0]] 
    ...
    def fillWhite(self, scores, coords, vis_level=-1, 
                   top_left=None, bot_right=None,
                   patch_size=(224, 224), 
                   blank_canvas=False, canvas_color=(220, 20, 50), alpha=0.4, 
                   blur=False, overlap=0.0, 
        ...
        for idx in range(len(coords)):
            ...
            if score >= threshold:
                ...
                # color_block = (cmap(raw_block) * 255)[:,:,:3].astype(np.uint8)
                shape = (raw_block.shape[0], raw_block.shape[1], 3)
                color_block = np.full(shape, (255, 255, 255) )
                # (0.5, 0, 0.5)
                # pdb.set_trace()
                # # Create a mask for the center area to be transparent
                # center_mask = np.zeros((patch_size[1], patch_size[0]), dtype=bool)
